...
fn run_basics(conn: &mut Connection, mode: EncodingMode) {
    with_zset_encoding(conn, mode, |conn| {
        ...
        assert!(redis::cmd("ZRANGEBYSCORE")
            .arg("zset")
            .arg(4)
            .arg(2)
            .query::<Vec<String>>(conn)
        ...
        assert!(redis::cmd("ZUNION")
            .arg(1)
            .arg("zseta")
            .query::<Vec<String>>(conn)
            .unwrap()
            .is_empty());
        assert!(redis::cmd("ZINTER")
            .arg(1)
            .arg("zseta")
            .query::<Vec<String>>(conn)
            .unwrap()
